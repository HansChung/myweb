class Fighter {
    ...
    move(keys) {
        ...
        if (keys[this.controls.super] && this.energy >= this.maxEnergy) {
            ...
            const superAttackDuration = 1000;
            const effectCount = 10;
            
            // 在 canvas 範圍內創建特效
            for (let i = 0; i < effectCount; i++) {
                setTimeout(() => {
                    // 計算特效位置（確保在角色周圍的合理範圍內）
                    const effectX = this.facingRight ? 
                        this.x + this.width + Math.random() * 100 : 
                        this.x - Math.random() * 100;
                    
                    const effectY = this.y + Math.random() * this.height;
                    
                    // 創建特效
                    this.createSuperEffect(effectX, effectY);
                    
                    // 檢查碰撞
                    const otherPlayer = this.playerNumber === "1" ? game.player2 : game.player1;
                    checkCollision(this, otherPlayer);
                    
                }, i * (superAttackDuration / effectCount));
            }
    ...
    getAttackBox() {
        ...
        let attackWidth = this.attackBox.width * attackScale.x;
        let attackOffsetX = this.attackBox.offsetX * attackScale.x;
        ...
        return {
            ...
            width: attackWidth,
            height: this.attackBox.height * (this.isSuper ? 1.5 : 1) // 增加高度
        };
    }

    onHit() {
        const damage = this.isSuper ? this.attackDamage * 2 : this.attackDamage;
        this.hitEffect = {
    ...
    drawEffects(ctx) {
        ...
        for (let i = this.effects.length - 1; i >= 0; i--) {
            ...
            const scale = 1 + progress; // 特效逐漸變大
            const currentSize = effect.size * scale;
            
            ctx.save();
            ctx.globalAlpha = 1 - progress; // 特效逐漸消失
            ctx.fillStyle = effect.color;
            ctx.beginPath();
            ctx.arc(effect.x, effect.y, currentSize/2, 0, Math.PI * 2);
            ctx.fill();
